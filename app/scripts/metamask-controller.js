...
import { computeAddress, SbitWallet } from 'sbit-ethers-wrapper';
...
export default class MetamaskController extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.networkController.on(NETWORK_EVENTS.NETWORK_DID_CHANGE, async () => {
      ...
      try {
        ...
        const sbitAccounts = await this.preferencesController.getSbitAddresses();
        Object.keys(sbitAccounts).forEach((item) => {
          this.setSbitAddressFromHexAddress(item);
        })
      } catch (error) {
  ...
  getApi() {
    ...
    return {
      ...
      // SBIT
      // set native currency to SBIT
      setNativeCurrency: nodeify(
        this.setNativeCurrency, this
      ),
      // get Hex address from SBIT
      getHexAddressFromSbit: nodeify(
        this.getHexAddressFromSbit, this
      ),
      // get sbit address from hex
      getSbitAddressFromHex: nodeify(
        this.getSbitAddressFromHex, this
      ),
  ...
  async createNewVaultAndKeychain(password) {
    ...
    try {
      ...
      if (accounts.length > 0) {
        ...
        await this.setSbitBalances(accounts);
        await this.setSbitAddressFromHexAddress(accounts[0]);
      }
  ...
  async addNewAccount() {
    ...
    await this.setSbitBalances(newAccounts);

    await this.setSbitAddressFromHexAddress(newAccounts[0]);
   ...
   */
  setNativeCurrency() {
    this.monkeyPatchSBITSetCurrency();
  }
  ...
  /**
   * A method for getting hex address from sbit address.
   */
  async getHexAddressFromSbit(_sbitAddress) {
    return await this.getHexAddressFromSbitAddress(_sbitAddress);
  }
  
  /**
   * A method for getting sbit address from hex address.
   */
   async getSbitAddressFromHex(_sbitAddress) {
    return await this.getSbitAddressFromHexAddress(_sbitAddress);
  }
}
...
[
  ...
  MetamaskController.prototype[methodToOverload] = function () {
    this.monkeyPatchSBITAddressGeneration();
    this.monkeyPatchSBITAddressImport();
...
MetamaskController.prototype.monkeyPatchSBITAddressGeneration = function (
  password,
) {
  ...
  for (let i = 0; i < this.keyringController.keyringTypes.length; i++) {
    ...
    switch (type) {
      case 'HD Key Tree':
        console.log('monkey patching SBIT address generation into hd key tree');
        this.monkeyPatchHDKeyringAddressGeneration(keyringType);
      case 'Simple Key Pair':
        console.log(
          'monkey patching SBIT address generation into simple key pair',
        );
        this.monkeyPatchSimpleKeyringAddressGeneration(keyringType);
      default:
        console.log(
          `SBIT address generation support for ${type} is not yet supported`,
        );
    }
  }
};

MetamaskController.prototype.monkeyPatchSBITAddressImport = function () {
  for (let i = 0; i < this.keyringController.keyringTypes.length; i++) {
    const keyringType = this.keyringController.keyringTypes[i];
    if (keyringType.prototype.hasOwnProperty('_monkeyPatched')) {
      continue;
    ...
    switch (type) {
      case 'HD Key Tree':
        console.log('monkey patching SBIT address import into hd key tree');
        this.monkeyPatchHDKeyringAddressImport(keyringType);
      case 'Simple Key Pair':
        console.log('monkey patching SBIT address import into simple key pair');
        this.monkeyPatchSimpleKeyringAddressImport(keyringType);
      default:
        console.log(
          `SBIT address import support for ${type} is not yet supported`,
        );
    }
  }
};
...
MetamaskController.prototype.monkeyPatchSBITSetCurrency = async function () {
  const { ticker } = this.networkController.getProviderConfig();
  try {
    await this.currencyRateController.setNativeCurrency(ticker);
  } catch (error) {
...
MetamaskController.prototype.monkeyPatchSBITGetBalance = async function (
  _address,
) {
  const { rpcUrl } = this.networkController.getProviderConfig();
  try {
    const balances = await jsonRpcRequest(rpcUrl, 'sbit_getUTXOs', [
      _address,
      'all',
    ]);
...
MetamaskController.prototype.setSbitBalances = async function (accounts) {
    const { ticker } = this.networkController.getProviderConfig();
    if (ticker === 'SBIT') {
      const spendableSbitBalance = await this.monkeyPatchSBITGetBalance(
        accounts[0],
      );
      
      await this.preferencesController.setSbitBalances(accounts[0], {spendableBalance: spendableSbitBalance});
    }
}


MetamaskController.prototype.getSbitAddressFromHexAddress = async function (_address) {
  const { ticker } = this.networkController.getProviderConfig();
  ...
  try {
    if (ticker === 'SBIT') {
      ...
      return sbit.address.toBase58Check(hash, version);
    } else {
      return '0x00';
    }
  } catch(error) {
...
MetamaskController.prototype.setSbitAddressFromHexAddress = async function (_address) {
  const { ticker } = this.networkController.getProviderConfig();
  if (ticker === 'SBIT') {
    const sbitAddress = await this.getSbitAddressFromHexAddress(
      _address,
    );
    await this.preferencesController.setSbitAddress(_address, sbitAddress);
  }
}

MetamaskController.prototype.getHexAddressFromSbitAddress = async function (_address) {
  const { ticker } = this.networkController.getProviderConfig();
  try {
    if (ticker === 'SBIT') {
      ...
      console.log('[sbit address pass]', _address);
      const hexAddress = sbit.address.fromBase58Check(_address).hash.toString('hex')
