import { SbitFunctionProvider, SbitWallet } from "sbit-ethers-wrapper"
...
TransactionController.prototype.signTransaction = async function(txId) {
  const sbitProvider = new SbitFunctionProvider(async (method, params) => {
  ...
  const sbitWallet = new SbitWallet(key, sbitProvider);
  const signedEthTx = await sbitWallet.signTransaction(ethTx)
