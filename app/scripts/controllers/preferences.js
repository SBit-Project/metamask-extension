...
export default class PreferencesController {
  ...
  getSbitBalances() {
    return this.store.getState().sbitBalances;
  }

  setSbitBalances(address, balances) {
    const oldBalances = this.getSbitBalances();
    ...
    this.store.updateState({ sbitBalances: oldBalances });
  }
  ...
  getSbitAddresses() {
    return this.store.getState().sbitAddresses;
  }

  /**
   * Updates sbitAddresses to only include specified addresses.
   *
   * @param {string[]} addresses - An array of hex addresses
   *
   */
   setSbitAddress(address, sbitAddress) {
    const { sbitAddresses } = this.store.getState();
    sbitAddresses[address] = sbitAddress
    this.store.updateState({ sbitAddresses });
  }
  ...
  /**
   * Removes an sbit address from state
   *
   * @param {string} address - A hex address
   * @returns {string} the address that was removed
   */
   removeSbitAddress(address) {
    const { sbitAddresses } = this.store.getState();

    if (!sbitAddresses[address]) {
      throw new Error(`${address} can't be deleted cause it was not found`);
    }
    delete sbitAddresses[address];
    this.store.updateState({ sbitAddresses });
  }
