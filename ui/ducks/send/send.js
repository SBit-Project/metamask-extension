...
export const initializeSendState = createAsyncThunk(
  ...
  async (_, thunkApi) => {
    ...
    if (asset.type === ASSET_TYPES.NATIVE) {
      sbitBalances = getSbitBalances(state);
    }
  ...
  reducers: {
    ...
    updateAmountToMax: (state) => {
      ...
      if (state.asset.type === ASSET_TYPES.TOKEN) {
        ...
        amount = subtractCurrencies(
          addHexPrefix(state.sbitBalances.spendableBalance),
    ...
    validateAmountField: (state) => {
      ...
      switch (true) {
        ...
        case state.asset.type === ASSET_TYPES.NATIVE && 
          !isBalanceSufficient({
            amount: state.amount.value,
            balance: sbitBalances.spendableBalance,
            gasTotal: state.gas.gasTotal ?? '0x0',
          }):
          state.amount.error = INSUFFICIENT_FUNDS_ERROR;
          state.sbitBalances.error = INSUFFICIENT_SPENDABLE_BALANCE_ERROR;
          break;
    ...
    validateGasField: (state) => {
      ...
      const insufficientFunds = !isBalanceSufficient({
        ...
        balance: state.asset.type === ASSET_TYPES.NATIVE ? sbitBalances.spendableBalance : state.account.balance,
        gasTotal: state.gas.gasTotal ?? '0x0',
      });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(initializeSendState.fulfilled, (state, action) => {
        ...
        state.gas.gasLimit = action.payload.gasLimit;
        state.sbitBalances = { ...action.payload.sbitBalances };
...
export function sendAmountIsInError(state) {
  return Boolean(state[name].sbitBalances.error);
}

export function sendSbitAmountIsInError(state) {
...
export function getSendErrors(state) {
  return {
    ...
    sbitBalances: state.send.sbitBalances.error
  };
}
...
export function getSbitSpendableBalanceInString(state) {
  let balance = '0x0'
  if(!!state.send.sbitBalances.spendableBalance) {
    const amount = subtractCurrencies(
      addHexPrefix(state.send.sbitBalances.spendableBalance),
