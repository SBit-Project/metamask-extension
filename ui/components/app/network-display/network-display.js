...
export default function NetworkDisplay({
  ...
  let isSbit = null;
  if (SBIT_PROVIDER_TYPES.includes(networkNickname)) {
    isSbit = networkNickname
    networkNickname = NETWORK_TO_NAME_MAP[networkNickname] || networkNickname;
  } else if (SBIT_PROVIDER_TYPES.includes(networkType)) {
    isSbit = networkType || networkNickname
  ...
  return (
    <Chip
      ...
      leftIcon={
        ...
        >
          <ColorIndicator
            color={networkType === NETWORK_TYPE_RPC ? (isSbit || COLORS.UI4) : networkType}
            size={indicatorSize}
