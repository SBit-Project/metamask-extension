...
export function getHexAddressFromSbitAddress(_address) {
  return async () => {
    try {
      const sbitAddress = await promisifiedBackground.getHexAddressFromSbit(_address);
      return sbitAddress;
    } catch (error) {
      console.log('[getHexAddressFromSbit error]', error);
      return undefined;
    }
  }
}

export function getSbitAddressFromHexAddress(_address) {
  return async () => {
    try {
      const sbitAddress = await promisifiedBackground.getSbitAddressFromHex(_address);
      return sbitAddress;
    } catch (error) {
      console.log('[getSbitAddressFromHexAddress error]', error);
      return undefined;
    }
  }
}
...
export function updateMetamaskState(newState) {
  return (dispatch, getState) => {
    ...
    if (nativeCurrency === 'SBIT' && newState.sbitBalances[newSelectedAddress] !== undefined) {
      dispatch({
        type: actionConstants.UPDATE_SBIT_BALANCE,
        value: {
          sbitBalances: {
            spendableBalance: newState.sbitBalances[newSelectedAddress].spendableBalance,
            pendingBalance: newState.sbitBalances[newSelectedAddress].pendingBalance,
          }
        }
      });
    }
...
export function setPrimaryAddressPreference(value) {
  return setPreference('isSbitAddressShow', value);
}
